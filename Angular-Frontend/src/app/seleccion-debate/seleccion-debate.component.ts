...
import {
  TeamsUsersService,
  DatosInsertarUsuarioEquipo,
} from '../services/teams-users.service';
...
export class SeleccionDebateComponent {
  ...
  constructor(
    ...
    private servicioUsuarioEquipo: TeamsUsersService
  ) {}
  ...
  async guardarDebate(nombreEquipo: string) {
    ...
    let usuarioEquipo = await firstValueFrom(
      this.servicioUsuarioEquipo.asignarUsuarioEquipo(datos)
    );
